# 🚀 Full Guide to Using Gemini

Everything you need to know to crush building anything with Gemini! This guide takes you from installation through advanced context engineering, subagents, hooks, and parallel agent workflows.

## 📋 Prerequisites

- Terminal/Command line access
- Node.js installed (for Gemini installation)
...
See detailed instructions in [install_gemini_windows.md](./install_gemini_windows.md)
...
## ✅ TIP 1: CREATE AND OPTIMIZE GEMINI.md FILES

Set up context files that Gemini automatically pulls into every conversation, containing project-specific information, commands, and guidelines.
...
Or create your own GEMINI.md file based on the template in this repository. See `GEMINI.md` for a Python specific example structure that includes:
...
**Power Keywords**: Gemini responds to certain keywords with enhanced behavior (information dense keywords):
- **IMPORTANT**: Emphasizes critical instructions that should not be overlooked
- **Proactively**: Encourages Gemini to take initiative and suggest improvements
...
- Prompt Gemini to write scripts to check its work: "After implementing, create a validation script"
- Avoid backward compatibility unless specifically needed - Gemini tends to preserve old code unnecessarily
- Focus on clarity and specific requirements rather than vague quality descriptors

### File Placement Strategies

Gemini automatically reads GEMINI.md files from multiple locations:

```bash
# Root of repository (most common)
./GEMINI.md              # Checked into git, shared with team
./GEMINI.local.md        # Local only, add to .gitignore

# Parent directories (for monorepos)
root/GEMINI.md           # General project info
root/frontend/GEMINI.md  # Frontend-specific context
root/backend/GEMINI.md   # Backend-specific context

# Reference external files for flexibility
echo "Follow best practices in: ~/company/engineering-standards.md" > GEMINI.md
```

**Pro Tip**: Many teams keep their GEMINI.md minimal and reference a shared standards document. This makes it easy to:
- Switch between AI coding assistants
- Update standards without changing every project
- Share best practices across teams

*Note: While Gemini reads GEMINI.md automatically, other AI coding assistants can use similar context files (such as .cursorrules for Cursor)*
...
When Gemini asks for permission, select "Always allow" for common operations.
...
Create `.gemini/settings.local.json`:
```json
{
  "allowedTools": [
    "Edit",
...
Slash commands are the key to adding your own workflows into Gemini. They live in `.gemini/commands/` and enable you to create reusable, parameterized workflows.

### Built-in Commands
- `/init` - Generate initial GEMINI.md
- `/permissions` - Manage tool permissions
- `/clear` - Clear context between tasks
- `/agents` - Manage subagents
- `/help` - Get help with Gemini
...
/primer
```
Performs comprehensive repository analysis to prime Gemini on your codebase so you can start implemention fixes or new features and it has all the necessary context to do so.
...
Commands can use `$ARGUMENTS` to receive parameters and can invoke any of Gemini's tools.
...
Connect Gemini to Model Context Protocol (MCP) servers for enhanced functionality. Learn more in the [Gemini documentation](https://ai.google.dev/docs).
...
gemini mcp add serena -- uvx --from git+https://github.com/oraios/serena serena start-mcp-server --context ide-assistant --project $(pwd)
```

[Serena](https://github.com/oraios/serena) transforms Gemini into a fully-featured coding agent with:
...
The more specific examples you provide, the better Gemini can match your existing patterns and style.
...
Subagents are specialized AI assistants that operate in separate context windows with focused expertise. They enable Gemini to delegate specific tasks to experts, improving quality and efficiency.
...
**Documentation Manager** (`.gemini/agents/documentation-manager.md`):
- Automatically updates docs when code changes
- Ensures README accuracy
- Maintains API documentation
- Creates migration guides

**Validation Gates** (`.gemini/agents/validation-gates.md`):
...
1. Use the `/agents` command or create a file in `.gemini/agents/`:
...
**4. Information Flow Design**: Understand how information flows from primary agent → subagent → primary agent. The subagent description is crucial because it tells your primary Gemini agent when and how to use it. Include clear instructions in the description for how the primary agent should prompt this subagent.
...
Learn more in the [Gemini documentation](https://ai.google.dev/docs).
...
Hooks provide deterministic control over Gemini's behavior through user-defined shell commands that execute at predefined lifecycle events.

### Available Hook Events

Gemini provides several predefined actions you can hook into:
...
This repository includes a simple hook example in `.gemini/hooks/`:

**log-tool-usage.sh** - Logs all tool usage for tracking and debugging:
```bash
#!/bin/bash
# Logs tool usage with timestamps
# Creates .gemini/logs/tool-usage.log
# No external dependencies required
```

### Setting Up Hooks

1. **Create hook script** in `.gemini/hooks/`
2. **Make it executable**: `chmod +x your-hook.sh`
3. **Add to settings** in `.gemini/settings.local.json`:
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": ".gemini/hooks/log-tool-usage.sh"
          }
        ]
      }
    ]
...
Set up the GitHub CLI to enable Gemini to interact with GitHub for issues, pull requests, and repository management.
...
Allow Gemini to perform any action while maintaining safety through containerization. This enables rapid development without destructive behavior on your host machine.
...
5. **Authenticate** Gemini in container
6. **Run in YOLO mode**:
   ```bash
...
Use Git worktrees to enable multiple Gemini instances working on independent tasks simultaneously, or automate parallel implementations of the same feature.
...
# Launch Gemini in each worktree
cd ../project-auth && gemini  # Terminal 1
cd ../project-api && gemini   # Terminal 2
```
...
- [Gemini Best Practices](https://ai.google.dev/docs/best-practices)
...
1. **Start Simple**: Set up GEMINI.md and basic permissions
...
Remember: Gemini is most powerful when you provide clear context, specific examples, and comprehensive validation. Happy coding! 🎉